...
public abstract class SyntaxNode(Token? startToken)
{
    ...
    /// </summary>
    public int LineNumber { get; set; } = startToken?.LineNumber ?? 0;
    ...
    public virtual async Task<bool> Visit([NotNull] ParsingContext context)
    {
        ...
        return token.Type switch
        {
            ...
            TokenType.Keyword => Constants.BooleanLiterals.Contains(token.Value)
                ? await this.VisitConstant(context)
                : await this.VisitKeyword(context),
            ...
            TokenType.NumberLiteral => await this.VisitConstant(context),
    ...
    /// <summary>
    /// Visits a constant literal <see cref="Token"/>.
    /// </summary>
    /// <param name="context">
    /// The context that describes and mutates the current state of the parser.
    /// </param>
    ...
    /// An awaitable <see cref="Task"/> that returns whether the <see cref="Token"/> was consumed.
    /// </returns>
    public virtual Task<bool> VisitConstant([NotNull] ParsingContext context) => Task.FromResult(false);
